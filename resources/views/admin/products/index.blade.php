...
<div class="p-4">
  <div class=" pb-2 mb-2 flex justify-between items-center">
    ...
    <a href="{{ url('admin/products/create') }}" class="px-8 py-2 bg-black text-white">Add Product</a>
  </div>
                        ...
                        @foreach ($products as $product)
                          <tr class="border-b dark:border-gray-700">
                            <td class="px-4 py-3">{{ $loop->iteration }}</td>
                              ...
                              <td class="px-4 py-3">
                                <img class="w-10 aspect-square object-contain" src="{{ asset('images/products/'.$product->image) }}" alt="user photo">
                              </td>
                              <td class="px-4 py-3">{{ $product->category_id }}</td>
                              <td class="px-4 py-3">{{ $product->brand }}</td>
                              <td class="px-4 py-3"><div class="line-clamp-3">{{ $product->description }}</div></td>
                              <td class="px-4 py-3">{{ $product->price }}</td>
                              <td class="px-4 py-3">
                                <div class="flex gap-2 h-full items-center justify-center">
                                  <a href="{{ url('admin/products/'.$product->id.'/view') }}" class="text-green-700">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-eye"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>
                                  </a>
                                  <a href="{{ url('admin/products/'.$product->id.'/edit') }}" class="text-blue-700">
                                  ...
                                  <a href="{{ url('admin/products/'.$product->id.'/delete') }}" class="text-red-700">
